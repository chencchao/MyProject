...
# 以下设备信息在配置文件中进行设置
Hub_Product_ID = 'KN3PQTREAH'
Hub_Device_Name = 'dev001'
Hub_Device_Key = 'fZNBsBy1hIQHpREsUCfvQg=='
Hub_REG_Product_ID = 'KN3PQTREAH'
Hub_REG_Product_Key = 'fZNBsBy1hIQHpREsUCfvQg=='
Hub_REG_Device_Name = 'dev001'
...
# 以下设备信息在配置文件中进行设置
IE_Product_ID = 'KN3PQTREAH'
IE_Device_Name = 'dev001'
IE_Device_Key = 'fZNBsBy1hIQHpREsUCfvQg=='
...
g_at_module_default_params = {
        ...
        'at_cmd_max_len': 2048,                         # 单条AT指令最大长度，单位：字节
...
class IoTBaseATCmd:
    ...
    def devinfo_setup(self, pid, dev_name, dev_key, tls=1):
        ...
        hint = cmd #'''AT+TCDEVINFOSET="%s","%s"''' % (pid, dev_name)
    ...
    def devinfo_query(self, product_id, device_name, key):
        ...
        print("Devinfo inconsistent rep:", rep)
        if rc:
            try:
    ...
    def publish_msg(self, topic, qos, msg, hint=''):
        ...
        cmd = '''AT+TCMQTTPUB="%s",%d,"%s"''' % (topic, qos, msg)
    ...
    def prdinfo_setup(self, pid, prd_key, dev_name, tls=1):
        ...
        #'''AT+TCPRDINFOSET="%s","%s"''' % (pid, dev_name)
        hint = cmd
...
class IoTHubATTest(IoTBaseATCmd):
    ...
    def test_case_mqtt_pub_recv(self, test_case_cnt):
        ...
        if not self.publish_normal_msg('data', 1, send_payload):
            ...
            cmd_err_cnt += 1
			  #'''%s/%s/data''' % (self.product_id, self.device_name)
        HubTestTopic = 'data'
        send_payload = get_hub_test_long_msg()
        if not self.publish_normal_msg(HubTestTopic, 1, send_payload):
            self.serial.output_record(self.report_file)
    ...
    def ota_update_test(self, version):
        ...
        if self.is_mqtt_connected():
            self.mqtt_disconnect()
        print("OTA update state1111")
        if not self.devinfo_setup(Hub_Product_ID, Hub_Device_Name, Hub_Device_Key):
            return False
        print("OTA update state2222")
        #if self.conn_mqtt_before_ota == 'yes':
        #   if not self.mqtt_connect():
        #        return False
        print("OTA update state3333")
        if not self.ota_update_setup(1, version):
            return False
        print("OTA update state4444")
...
class AILINKWiFiATCmd:
    ...
    def is_network_connected(self):
        ...
        ret, rep = self.serial.send_cmd_wait_reply(cmd, ok_reply, self.err_list, self.cmd_timeout)
        print("Network is connected rep", rep)
        if ret:
            try:
                ip_str = rep.split('\n', 1)[0]
                status = ip_str.find('+ATW:ip:"0.0.0.0"')
                if status == -1:
    ...
    def wificonfig_msg_handler(self, msg):
        try:
            status = msg.split(':', 1)[1]
        except ValueError:
            print("\nInvalid wificonfig msg:", msg)
            return
    ...
    def start_wifi_config(self):
        # AT+TCWIFICONFIG"
        cmd = 'AT+TCWIFICONFIG'
        ok_reply = '+TCWIFICONFIG:OK'
        hint = cmd

        if not self.serial.do_one_at_cmd(cmd, ok_reply, hint, self.err_list, 10):
            return False

        self.serial.add_data_handler("+TCWIFICONFIG", self.wificonfig_msg_handler)
        self.boarding_state = 'boarding'
        return True

    def stop_wifi_config(self):
        # AT+TCSTOPWIFICONFIG"
        cmd = 'AT+TCSTOPWIFICONFIG'
        ok_reply = '+TCSTOPWIFICONFIG:OK'
        hint = cmd
...
def at_module_network_connect(at_module, WiFi_SSID, WiFi_PSWD):
    if at_module == 'ESP8266':
        return AILINKWiFiATCmd().do_network_connection(WiFi_SSID, WiFi_PSWD)
...
def main():
    ...
    if not test_mode in ['CLI', 'MQTT', 'IOT', 'WIFI', 'HUB', 'OTA', 'IE', 'CERT', 'WIFI_CONFIG', 'GW']:
    ...
    if at_module == 'ESP8266':
        AILINKWiFiATCmd().set_tc_log_debug(log_level)

    while True:
        # WiFi boarding test
        if test_mode == 'WIFI':
            AILINKWiFiATCmd().wifi_boarding_test()
            break
        if test_mode == 'WIFI_CONFIG':
            AILINKWiFiATCmd().wifi_config_test()
            break
